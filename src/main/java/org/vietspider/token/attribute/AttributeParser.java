...
//
  @Deprecated()
  public static Attributes getAttributes(Node<?> node) { return parse(node); }

  @Deprecated()
  public static Attribute[] getAttributes(String text) { return parse(text); }
  ...
  final static private void parseStyle(Attributes list) {
    ...
    parseStyle(list, value);
  }
  
  final static public void parseStyle(Attributes list, String value) {
    if(value == null) return;
    List<String> elements = CharsUtil.split2List(value, ';');
//    String [] elements = value.split(";");
    for(int i = 0; i < elements.size(); i++) {
      String element  = elements.get(i);
      if((element = element.trim()).isEmpty()) continue;
      List<String> data = CharsUtil.split2List(value, ':');
      if(data.size() < 1) {
        continue;
      } else if(data.size() < 2) {
...
//    System.out.println(single+ " : "+ value + " : " + text);
  ...
  private static boolean isSingle(String text, int index) {
    int length = text.length();
    while(index < length) {
      char c = text.charAt(index);
      if(Character.isWhitespace(c) 
  ...
  private static Object [] getValue(String text) {
